...
bool CalcularCombinacionOptima(int PrimeraCombinacion, int UltimaCombinacion, PtrListaArboles Optimo)
{
	long int Combinacion, MejorCombinacion=0, CosteMejorCombinacion;
	int Coste;
	TListaArboles OptimoParcial;
	...
	//reservar memoria para el array de structs
	RangoCombinaciones = malloc (sizeof(struct Rango)*num_threads);
	if (threads==NULL)  {
		perror("Error reserva memoria rango combinaciones."); 
		exit(1);
	}
	...
	for(int i=0; i<num_threads; ++i) {
		Coste = EvaluarCombinacionListaArboles(RangoCombinaciones[i].MejorCombinacion);
		if ( Coste < CosteMejorCombinacion )
		{
			CosteMejorCombinacion = Coste;
			MejorCombinacion = RangoCombinaciones[i].MejorCombinacion;
		}
		if ((RangoCombinaciones[i].MejorCombinacion%S)==0)
		{
			 ConvertirCombinacionToArbolesTalados(MejorCombinacion, &OptimoParcial);
			 printf("\r[%ld] OptimoParcial %ld-> Coste %ld, %d Arboles talados:", RangoCombinaciones[i].MejorCombinacion, MejorCombinacion, CosteMejorCombinacion, OptimoParcial.NumArboles);
	...
	ConvertirCombinacionToArbolesTalados(MejorCombinacion, &OptimoParcial);
	printf("\rOptimo %ld-> Coste %ld, %d Arboles talados:", MejorCombinacion, CosteMejorCombinacion, OptimoParcial.NumArboles);
	MostrarArboles(OptimoParcial);
	printf("\n");

	if (CosteMejorCombinacion == Optimo->Coste)
		return false;  // No se ha encontrado una combinacion mejor.

	// Asignar combinacion encontrada.
	ConvertirCombinacionToArbolesTalados(MejorCombinacion, Optimo);
	Optimo->Coste = CosteMejorCombinacion;
	// Calcular estadisticas optimo.
...
void CombinacionOptimaThread(struct Rango *RangoCombinaciones) {
	for (long int Combinacion=RangoCombinaciones->combinacion_inicial; Combinacion<RangoCombinaciones->combinacion_final; ++Combinacion) {
		RangoCombinaciones->Coste = EvaluarCombinacionListaArboles(Combinacion);
		...
		if ((Combinacion%S)==0) {
			 ConvertirCombinacionToArbolesTalados(RangoCombinaciones->MejorCombinacion, &RangoCombinaciones->OptimoParcial);
			 printf("\r[%ld] OptimoParcial %ld-> Coste %ld, %d Arboles talados:", Combinacion, RangoCombinaciones->MejorCombinacion, RangoCombinaciones->CosteMejorCombinacion, RangoCombinaciones->OptimoParcial.NumArboles);
			 MostrarArboles(RangoCombinaciones->OptimoParcial);
		}
	}
}
